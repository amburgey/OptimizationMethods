...
createData <- function(type, nsims, Ngroup, Nsnsz, stat, VISloc, TRAPloc){
  ...
  if(type == "VISTRAP"){
    ...
    hist(p0MV[[2]], col = cV2, add = TRUE, breaks = 25)
    hist(p0MV[[3]], col = cV3, add = TRUE, breaks = 25)
    hist(p0MV[[4]], col = cV4, add = TRUE, breaks = 25)
    hist(p0MT[[1]], col = cT1, add = TRUE, breaks = 25)
    hist(p0MT[[2]], col = cT2, add = TRUE, breaks = 25)
    hist(p0MT[[3]], col = cT3, add = TRUE, breaks = 25)
    hist(p0MT[[4]], col = cT3, add = TRUE, breaks = 25)
    ...
    for(z in 1:nsims){  
      ...
      for(n in 1:N){
        yTrueVIS[n,,z] <- rpois(J1,pmatV[n,newX1]*K)  # observations of each snake at each location based on encounter probability and effort
