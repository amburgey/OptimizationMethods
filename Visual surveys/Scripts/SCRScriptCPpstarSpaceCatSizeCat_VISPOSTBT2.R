...
source("Visual surveys/DataPrep/DataPrepCP_VISPOSTBT2.R")              ## Functions to reshape survey and capture data
...
CPcaps <- subset(CPcaps, PROJECTCODE == "POST BT2 VIS")
CPsurv <- subset(CPsurv, PROJECTCODE == "POST BT2 VIS")

## SECIFY TIME FRAME
time <- c("04","05")
time2 <- c("2009-05-01","2010-07-30")
...
save(out, file="Visual surveys/Results/NWFNVISPOSTBT2_SCRpstarvisCATsizeCATupdatedgrid10.Rdata")
# save(out, file="Visual surveys/Results/NWFNVISPOSTBT2_SCRpstarvisCATNOSIZEgrid10.Rdata")
