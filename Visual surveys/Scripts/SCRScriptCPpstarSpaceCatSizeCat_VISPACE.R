...
source("Visual surveys/DataPrep/DataPrepCP_VISPACE.R")              ## Functions to reshape survey and capture data
...
CPcaps <- subset(CPcaps, PROJECTCODE == "NWFN VISPACE")
CPsurv <- subset(CPsurv, PROJECTCODE == "NWFN VISPACE")

## SECIFY TIME FRAME
time <- c("05","06")
time2 <- c("2016-04-01","2016-07-30")
...
inits <- function(){
  list (sigma=runif(1,30,40), n0=c(300,27,57,300), s=vsst, p0=runif(L,.002,.003))
}
...
save(out, file="Visual surveys/Results/NWFNVISPACE_SCRpstarvisCATsizeCAT.Rdata")  ## M = 150 (XXXXhrs)
