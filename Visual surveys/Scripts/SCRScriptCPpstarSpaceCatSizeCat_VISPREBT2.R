...
source("Visual surveys/DataPrep/DataPrepCP_VISPREBT2.R")              ## Functions to reshape survey and capture data
...
CPcaps <- subset(CPcaps, PROJECTCODE == "PRE BT2 VIS")
CPsurv <- subset(CPsurv, PROJECTCODE == "PRE BT2 VIS")

## SECIFY TIME FRAME
time <- c("02","03")
time2 <- c("2008-08-01","2009-04-30")
...
## MCMC settings
nc <- 3; nAdapt=100; nb <- 100; ni <- 100+nb; nt <- 1  ## hits error at 2000 iter, 1000 adapt
...
save(out, file="Visual surveys/Results/NWFNVISPREBT2_SCRpstarvisCATsizeCAT.Rdata")  ## M = 150 (XXXXhrs)
