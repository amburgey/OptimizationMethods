...
ngroupall <- ngroupall
ngroupall <- rbind(ngroupall,matrix(0,0,nrow=1,ncol=2))
...
snszall[is.na(snszall)] <- 5  ## no animal captured
...
model {
  ...
  for(t in 1:nproj){
    ...
    for(i in 1:nmax){  ## nmax = maximum number of observed individuals across all projects
      ...
      for(j in 1:J){  ## J = number of traps
        ...
        p[i,j,t] <- p0[size[i,t]]*exp(-alpha1*Gdist[s[i,t],j]*Gdist[s[i,t],j])*nFound[i,t]
      }#J
    }#I
...
",file = "Visual surveys/Models/SCRpstarCATsizeCAT_CPALLtest.txt")

#######################################################

## MCMC settings
nc <- 3; nAdapt=10; nb <- 1; ni <- 10+nb; nt <- 1

## Data and constants
jags.data <- list (y=yall, Gpts=Gpts, Gdist=Gdist, J=J, locs=X, A=A, K=Kall, nFound=nFound, nmax=nrow(yall), a=a, dummy=matrix(0,nrow=(Lall+1),ncol=nproj), b=rep(1,Gpts), size=snszall, L=(Lall+1), ngroup=ngroupall, nproj=nproj) # ## semicomplete likelihood, #nFound=nFound, nmax=max(nindall), 

inits <- function(){
  list (sigma=runif(1,30,40), n0=(ngroupall+10), s=vsstall, p0=runif((L+1),.002,.003)) 
}

parameters <- c("p0","sigma","pstar","alpha0","alpha1","N","n0","Ngroup","piGroup")

out <- jags("Visual surveys/Models/SCRpstarCATsizeCAT_CPALLtest.txt", data=jags.data, inits=inits, parallel=TRUE,
...
save(out, file="Visual surveys/Results/NWFNVISALL_SCRpstarvisCATsizeCATdpois10GRIDvector.Rdata")
