...
CPcaps <- subset(CPcaps, PROJECTCODE == "POST KB TRAP 2")
CPsurv <- subset(CPsurv, PROJECTCODE == "POST KB TRAP 2")

## SECIFY TIME FRAME
time <- c("05","07") ## actually only 2 months due to late start sampling in May
time2 <- c("2013-04-01","2013-08-31")
...
snsz <- ifelse(snsz < 850, 1,
               ifelse(snsz >= 950 & snsz < 1150, 2,   ## removed 2nd category as there are none and renumbered
...
## For Size model:
save(out, file="Trapping/Results/NWFNPOSTKBTRAP2_SCRpstartrapCATsizeCAT.Rdata")
## For No Size model:
