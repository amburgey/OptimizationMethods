##### ASO (Andy South) is an open (no fence) study area near swiftlet caves in the south

## The swift array study was done to see if concenric rings and webs of traps could pick up snakes as they move towards/from a central bird prey trap

rm(list=ls())

source("Select&PrepTrapData.R")   ## Creation of subcap and subsurv (cleaned up)
source("Trapping/DataPrep/DataPrepASO.R")    ## Functions to reshape survey and capture data
source("Trapping/DataPrep/OverlayASOGrid.R")

library(secr); library(reshape2); library(jagsUI)

## Subset capture data (subcap) and effort/survey data (subsurv)
ASOcaps <- subset(subcap, SITE == "ASOI")
ASOsurv <- subset(subsurv, SITE == "ASOI")

## Subset to specific NWFN project
ASOcaps <- subset(ASOcaps, PROJECTCODE == "SWIFT TRAP ARRAY")
ASOsurv <- subset(ASOsurv, PROJECTCODE == "SWIFT TRAP ARRAY")

## SECIFY TIME FRAME
time <- c("03","04") ## this sampling was just the end of June to beginning of August so not over 2 month rule
time2 <- c("2014-02-01","2014-05-31")


##### SPECIFY DIMENSIONS OF CP #####
cellsize <- c(10,10)  ## dimensions of integration grid cell
ASOspecs <- overlayCP(ASOcaps, cellsize)  ## ignore warnings, all about projections
## Area (5 ha/50,000 m2): 
A <- 50000
