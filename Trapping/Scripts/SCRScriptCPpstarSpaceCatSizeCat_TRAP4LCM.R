...
CPcaps <- subset(CPcaps, PROJECTCODE == "NWFN TRAP 4 LCM")
CPsurv <- subset(CPsurv, PROJECTCODE == "NWFN TRAP 4 LCM")

## SECIFY TIME FRAME
time <- c("08","09")
time2 <- c("2007-07-01","2007-10-31")
...
colnames(act) <- NULL
K <- rowSums(act)
## for running combined analysis of all projects
K <- append(rep(0,13),K,after=13)
...
# 
# ## For Size model:
# save(out, file="Trapping/Results/NWFNTRAP4LCM_SCRpstartrapCATsizeCAT.Rdata")
# ## For No Size model:
