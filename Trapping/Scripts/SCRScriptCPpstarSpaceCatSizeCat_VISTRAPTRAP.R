...
CPcaps <- subset(CPcaps, PROJECTCODE == "NWFN VISTRAP TRAP")
CPsurv <- subset(CPsurv, PROJECTCODE == "NWFN VISTRAP TRAP")

## SECIFY TIME FRAME
time <- c("02","03")
time2 <- c("2007-01-01","2007-04-31")
...
## MCMC settings
nc <- 3; nAdapt=200; nb <- 100; ni <- 3500+nb; nt <- 1
...
## For Size model:
save(out, file="Trapping/Results/NWFNVISTRAPTRAP_SCRpstartrapCATsizeCAT3500.Rdata")
## For No Size model:
