...
scen <- c(
  ...
  "closedTRAP120small14full","closedTRAP120small14half",
  "closedTRAP60small30full","closedTRAP60small30half",
  "closedTRAP60small14full","closedTRAP60small14half",
  "closedTRAP120large30full","closedTRAP120large30half",
...
listfiles <- list.files("D:/Results/")
...
for(t in 1:length(scen)){ #1:length(scen)
  ...
  if(str_detect(scen[t], "120")){
    simSumm[count,9] <- mean(sqrt(var(out$sims.list$N) + (out$sims.list$N - 120)^2))  # RMSE for 120 snakes in simulation
  } else {
    simSumm[count,9] <- mean(sqrt(var(out$sims.list$N) + (out$sims.list$N - 60)^2))  # RMSE for 60 snakes in simulation
  }
