...
####### ONLY FOR TESTING ##############
perf2 <- rbind(perf,perf)
perf2$Sim <- c("closedVISTRAP120small60half","closedTRAP60small60half","closedVIS60small60half","closedVISTRAP60small60half")
...
startplot <- ggplot(startup, aes(x = Scenario, y = TotalStart, fill = Type)) +
...
pareto1 <- ggplot(perfcost, aes(x = TotalRun, y = RMSE, fill = Type.x)) +
