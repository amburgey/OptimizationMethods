...
subsurv <- subset(subsurv, PROJECTCODE %in% c("HMU TOX DROP 2 TRAP", "TOX DROP TRAP","KB TRAP","POST KB TRAP 1","POST KB TRAP 2", "NWFN TRAP 1","NWFN TRAP 2 LINVIS","NWFN TRAP 3","NWFN TRAP 4 LCM","NWFN VISTRAP TRAP","PRE BT1 TRAP","POST BT2 TRAP","NWFO TRAP 1","NWFO TRAP 2","NWFO TRAP 3","NWFO TRAP 4","GNWR SSP MOUSE", "GNWR SSP BIRD", "SWIFT TRAP ARRAY"))
...
## 372 traps have some kind of comment on them about failure
test <- subset(subsurv, COMMENTS != "")
...
## This represents about 3.8% of records that have some potential issue with trap performance (across all projects)
fail <- read.csv("/Users/Staci Amburgey/Documents/USGS/BrownTreesnakes/Optim Monitoring Methods/Data/TrapNonfunctional.csv")
failtrap <- merge(test[,c(1:5,13,22)],fail, by=c("EFFORTID","TRANSID"), all.x=TRUE)
failtrap <- merge(subsurv[,c(1:5,13,22)],fail, by=c("EFFORTID","TRANSID"), all.x=TRUE)
test2 <- subset(failtrap, !is.na(TRAPROW))
## leaving for right now until better understood
...
## Projects deemed suitable (marked snakes, enough recaptures, closed period of time, spatial information, etc.)
subcap <- subset(subcap, PROJECTCODE %in% c("HMU TOX DROP 2 TRAP", "TOX DROP TRAP","KB TRAP","POST KB TRAP 1","POST KB TRAP 2", "NWFN TRAP 1","NWFN TRAP 2 LINVIS","NWFN TRAP 3","NWFN TRAP 4 LCM","NWFN VISTRAP TRAP","PRE BT1 TRAP","POST BT2 TRAP","NWFO TRAP 1","NWFO TRAP 2","NWFO TRAP 3","NWFO TRAP 4","GNWR SSP MOUSE", "GNWR SSP BIRD", "SWIFT TRAP ARRAY"))
